...
func resolveMusicMetadata(ctx context.Context, raw string) (musicMetadata, error) {
	...
	case "spotify":
		if result.ExternalID != "" && os.Getenv("SPOTIFY_CLIENT_ID") != "" && os.Getenv("SPOTIFY_CLIENT_SECRET") != "" {
			if data, err := spotifyMetadata(ctx, kind, result.ExternalID); err == nil {
				mergeMetadata(&result, data)
				return result, nil
			}
		}
		if data, err := oEmbedMetadata(ctx, "https://open.spotify.com/oembed?url="+url.QueryEscape(canonical)); err == nil {
			mergeMetadata(&result, data)
		}
	case "youtube":
		if result.ExternalID != "" && os.Getenv("YOUTUBE_API_KEY") != "" {
			if data, err := youtubeMetadata(ctx, kind, result.ExternalID); err == nil {
				mergeMetadata(&result, data)
				return result, nil
			}
		}
		if kind == "video" {
			if data, err := oEmbedMetadata(ctx, "https://www.youtube.com/oembed?format=json&url="+url.QueryEscape(canonical)); err == nil {
				mergeMetadata(&result, data)
			}
		}
	}
...
func canonicalMusicURL(raw string) (string, *url.URL, error) {
	...
	case strings.Contains(u.Host, "youtube.com"):
		...
		} else if clean.Get("list") != "" && clean.Get("v") == "" {
			u.Path = "/playlist"
		} else {
			u.Path = "/watch"
		}
...
func oEmbedMetadata(ctx context.Context, endpoint string) (musicMetadata, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return musicMetadata{}, fmt.Errorf("metadata unavailable")
	}
...
func spotifyMetadata(ctx context.Context, kind, id string) (musicMetadata, error) {
	...
	if resp.StatusCode != http.StatusOK || json.NewDecoder(resp.Body).Decode(&token) != nil {
		return musicMetadata{}, fmt.Errorf("spotify authentication failed")
	}
	resource := map[string]string{"track": "tracks", "album": "albums", "playlist": "playlists", "artist": "artists"}[kind]
	if resource == "" {
		return musicMetadata{}, fmt.Errorf("unsupported spotify resource")
	}
	...
	if resp.StatusCode != http.StatusOK || json.NewDecoder(resp.Body).Decode(&payload) != nil {
		return musicMetadata{}, fmt.Errorf("spotify metadata unavailable")
	}
	...
	} else if len(payload.Album.Images) > 0 {
		image = payload.Album.Images[0].URL
	}
	artist := strings.Join(artists, ", ")
	if artist == "" {
		artist = payload.Owner.DisplayName
	}
...
func youtubeMetadata(ctx context.Context, kind, id string) (musicMetadata, error) {
	...
	endpoint := fmt.Sprintf("https://www.googleapis.com/youtube/v3/%s?part=snippet&id=%s&key=%s", resource, url.QueryEscape(id), url.QueryEscape(os.Getenv("YOUTUBE_API_KEY")))
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	resp, err := metadataHTTPClient.Do(req)
	if err != nil {
		return musicMetadata{}, err
	...
	if resp.StatusCode != http.StatusOK || json.NewDecoder(resp.Body).Decode(&payload) != nil || len(payload.Items) == 0 {
		return musicMetadata{}, fmt.Errorf("youtube metadata unavailable")
	}
	s := payload.Items[0].Snippet
