...
func (a *api) resetOwnerPassword(w http.ResponseWriter, r *http.Request) {
	...
	expectedToken := strings.TrimSpace(os.Getenv("ADMIN_SETUP_TOKEN"))
	providedToken := strings.TrimSpace(in.SetupToken)
	if expectedToken == "" || subtle.ConstantTimeCompare([]byte(subtleToken(providedToken)), []byte(subtleToken(expectedToken))) != 1 {
